...
public class NativeTestExecuter implements TestExecuter {
    ...
    public void execute(Test testTask, TestResultProcessor testResultProcessor) {
        ...
        TestClassProcessor processor = new NativeTestClassProcessor(executable, workingDir, getExecHandleBuilder(), getIdGenerator(), getTimeProvider());
        ...
        System.out.println(testTaskOperationId);
    ...
    private static class NativeTestClassProcessor implements TestClassProcessor {
        ...
        NativeTestClassProcessor(File executable, File workingDir, ExecHandleBuilder execHandleBuilder, IdGenerator<?> idGenerator, TimeProvider timeProvider) {
            this.execHandleBuilder = execHandleBuilder;
            this.idGenerator = idGenerator;
            this.timeProvider = timeProvider;
            this.bundle = executable;
            execHandleBuilder.executable("/Applications/Xcode.app/Contents/Developer/usr/bin/xctest");
    ...
    private static class TextStreamToProcessor implements TextStream {
        ...
        public void text(String text) {
            ...
            if (text.startsWith("Test Suite")) {
                Matcher m = TEST_SUITE_NAME_PATTERN.matcher(text);
                if (!m.find()) {
                    return;
                }
                String testSuite = m.group(1);

                if (text.contains("started at")) {
                    TestDescriptorInternal testDescriptor = new DefaultTestSuiteDescriptor(idGenerator.generateId(), testSuite);
                ...
                Matcher m = TEST_CASE_NAME_PATTERN.matcher(text);
                m.find();
                String testSuite = m.group(1);
                String testCase = m.group(2);
