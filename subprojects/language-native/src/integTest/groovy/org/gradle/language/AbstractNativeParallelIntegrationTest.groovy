...
abstract class AbstractNativeParallelIntegrationTest extends AbstractInstalledToolChainIntegrationSpec {
    ...
    void assertTaskIsParallel(String taskName) {
        ...
        def parallelTasks = buildOperations.getOperationsConcurrentWith(ExecuteTaskBuildOperationType, task)
        assert parallelTasks.size() > 0
        assert parallelTasks.find { it.displayName == "Task :parallelTask" }
    }
